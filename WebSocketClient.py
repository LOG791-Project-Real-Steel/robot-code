...
"""
import gi, argparse, json, asyncio, websockets, time, signal, statistics
...
UPGRADE_RTT_MAX   = 100 / 1000  # seconds -  avg RTT < 100 ms  → step _up_
DOWNGRADE_RTT_MIN = 200 / 1000  # seconds - avg RTT > 200 ms  → step _down_
MAX_BACKOFF       = 30          # seconds - reconnect cap
HIGH_WATER        = 256 * 1024  # bytes
SKIP_LIMIT        = 2           # skipped frames before profile downgrade
...
class Camera:
    ...
    def reconfigure(self, width, height, quality):
...
async def send_frames(ws):
    ...
    while True:
        ...
        if MODE_AUTO and buffer_size > HIGH_WATER:
            ...
            if skipped >= SKIP_LIMIT and quality_idx + step < len(QUALITIES) - 1:
                async with quality_lock:
                    quality_idx += step
                    cam.reconfigure(QUALITIES[quality_idx])
...
async def main():
    global cam
    width, height, quality = QUALITIES[quality_idx]
    if not MODE_AUTO:
...
# -- Graceful Ctrl-C --
def all_tasks(loop):
    return asyncio.Task.all_tasks(loop) # For python 3.6
